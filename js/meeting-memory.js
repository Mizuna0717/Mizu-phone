...
async function mtgWriteToMemoryLibrary(sessionId) {
  ...
  stm.content,
  { source: 'meeting', meetingId: session.id, meetingName: session.name }   // ★ 新增
);
...
state.memories.push({
  ...
  autoGenerated: true,
  source: 'meeting',                        // ★ 新增
  meetingId: session.id,                    // ★ 新增
  meetingName: session.name,                // ★ 新增
  ...
  if (typeof showToast === 'function') {
    ...
    // ★ 标记此 session 的内容已写入记忆库（用于 iMessage 上下文挂载判断）
  if (writtenCount > 0) {
    session._memoryWritten = true;
    session._memoryWrittenAt = Date.now();
  }
  saveState();
