...
;(function() {
  ...
  window.openMessageChat = function(chatId) {
    ...
    if(chat.messages&&chat.messages.length){
      ...
      chat.messages.forEach(function(msg,idx){
        ...
        var isSent=(msg.sender==='user'), isGF=(msg.sender!==prevSender);
        ...
        prevSender=msg.sender; prevTs=msg.timestamp||0;
      });
  ...
  window.rollMessageChats = async function() {
    ...
    var npcList=selectedNpcs.map(function(npc,idx){var name=npc.name||('NPC_'+idx);var pers=npc.personality||npc.systemPrompt||npc.notes||npc.desc||'a friendly person';var nick=npc.nickname||npc.displayName||'';return(idx+1)+'. Name: "'+name+'"'+(nick?', Nickname: "'+nick+'"':'')+', Personality: "'+pers+'"';}).join('\n');
    var userName=(state.userProfile&&state.userProfile.name)||'User';
    var prompt='You are generating realistic text-message chat histories. The user is "'+userName+'".\n\n'+(usingFictional?'These are fictional contacts:\n':'NPCs:\n')+npcList+'\n\nRULES:\n1. For EACH person, generate a separate conversation with 3-8 messages.\n2. Messages alternate between "user" and "npc".\n3. Content must be natural and fit the personality.\n4. Use the language matching the name.\n5. Each conversation should have a coherent topic.\n\nReturn ONLY a valid JSON array:\n[\n  { "npcIndex": 0, "messages": [ { "sender": "user", "content": "..." }, { "sender": "npc", "content": "..." } ] }\n]';
    try {
      ...
      convos.forEach(function(convo,ci){
        ...
        var stamped=msgs.map(function(m,mi){return{sender:(m.sender==='user')?'user':'npc',content:(m.content||'').trim(),timestamp:baseTime+mi*(60000+Math.floor(Math.random()*300000))};}).filter(function(m){return m.content.length>0;});
