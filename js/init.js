...
(function() {
  ...
  //  封装全部初始化逻辑
  //  — 首次加载时直接执行
  //  — 认证成功后由 auth.js 通过 window.__runAppInit() 再次调用
  // ═══════════════════════════════════════════════
  function runAppInit() {
    ...
    if (window.__authRequired) {
      console.log('[init] Auth required — skipping app initialization');
      // 仍然触发 appReady 以清除安全网计时器
      try { window.dispatchEvent(new Event('appReady')); } catch(e) {}
      return;
    }
    ...
    // 认证成功后同步 state.user
    if (window.__user && typeof state !== 'undefined') {
      state.user = window.__user;
    }

    // 加载后立即做数据快照，用于检测后续是否被意外重置
    ...
    // 验证 initSystemPrompts 没有破坏数据
    if (state.characters.length !== _postLoadChars) {
      console.error('[init] initSystemPrompts 后角色数量变化！',
    ...
    deferredRenders.forEach(function(renderFn) {
      scheduleIdle(function() {
        ...
        if (state.characters.length !== _postLoadChars && _postLoadChars > 0) {
          console.error('[init] 延迟渲染后角色数量变化！',
            _postLoadChars, '->', state.characters.length,
    ...
    // 显示认证用户
    if (window.__user) {
      console.log('[init] 已认证用户: ' + window.__user.name + ' (QQ: ' + window.__user.qq + ')');
    }
  ...
  // ═══════════════════════════════════════════════
  //  导出初始化函数供 auth.js 登录成功后调用
  // ═══════════════════════════════════════════════
  window.__runAppInit = runAppInit;
